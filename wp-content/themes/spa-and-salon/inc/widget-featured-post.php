...
if( ! class_exists( 'Spa_and_Salon_Featured_Post' ) ): 
 /**
...
class Spa_and_Salon_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'RARA: Featured Post', 'spa-and-salon' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'spa-and-salon' ), ) // Args
		);
	}
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'spa-and-salon-featured-post', array( 'itemprop' => 'image' ) ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'spa-and-salon' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'spa-and-salon' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'spa-and-salon' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'spa-and-salon' );
